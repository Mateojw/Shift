...
public class GameManager : MonoBehaviour {
  ...
  //Used to tell the PlayerController script how much to rotate 90 or -90 degrees depending on input
      ...
      if(facingDirection == FacingDirection.Front || facingDirection == FacingDirection.Back)
      {
        ...
        if(Mathf.Abs(item.position.z - PlayerController.transform.position.z) < WorldUnits + 0.1f)
        {
          if(PlayerController.transform.position.y - item.position.y <= WorldUnits + 0.2f && PlayerController.transform.position.y - item.position.y >0 && !PlayerController._jumping)
          {
            ...
            if(facingDirection == FacingDirection.Left && item.position.x < PlayerController.transform.position.x)
              moveCloser = true;
  ...
  private float GetPlayerDepth()
  {
    ...
    if(facingDirection == FacingDirection.Front || facingDirection == FacingDirection.Back)
    {
      ClosestPoint = PlayerController.transform.position.z;

    }
    else if(facingDirection == FacingDirection.Right || facingDirection == FacingDirection.Left)
    {
      ClosestPoint = PlayerController.transform.position.x;
    }
